...
4. 点击「启动服务」显示悬浮控制面板
5. 点击「+」添加触控目标，拖动到目标位置
6. 编辑模式下点按目标点，切换单次触控 / 持续触控（两种类型可混合使用；绿角标=单次，橙角标=持续）
7. 点击面板上的设置按钮，配置全局触控参数（间隔时间、滑动距离与方向，对所有目标生效）
8. 点击面板上的方案按钮，可将当前配置（全局参数 + 目标数量/位置/类型）保存为方案，并在方案列表中快速切换
9. 点击播放按钮开始自动触控
10. 点击停止按钮停止当前任务
11. 点击退出按钮关闭服务（需确认）
...
- 支持最多 **5 个独立触控目标**，间隔与滑动参数共享全局配置
- 触控类型**按目标独立设置**：编辑模式下点按目标点即可切换单次/持续触控，两种类型可混合存在
- 支持**配置方案管理**：当前配置一键保存为方案，方案列表快速切换
- 支持悬浮控制面板，可拖拽移动
- 支持触控目标可视化拖拽定位
- 全局参数通过面板上的设置按钮统一配置：**单次触控**的间隔时间、**持续触控**的滑动方向和距离
...
data class ClickTargetInfo(
    ...
    val x: Float,
    val y: Float,
    val clickType: ClickType,   // SINGLE | LONG_PRESS，按目标独立设置
    val interval: Long,          // 单次触控间隔(ms)，全局
    val swipeDistance: Int,      // 滑动距离(px)，全局
    val swipeAngle: Int          // 滑动方向角度(0=右,90=下,180=左,270=上)，全局
)
```
...
职责：
- 管理悬浮控制面板和多个触控目标视图
- 处理拖拽、编辑模式、全局设置弹窗（面板设置按钮）
- 管理配置方案的保存、切换与删除（面板方案按钮）
- 持久化保存目标坐标、全局触控设置与配置方案
- 与 AutoClickService 同步目标数据（统一填充全局设置）

### 4. ConfigProfile
文件：`app/src/main/java/com/jons/touchassist/core/ConfigProfile.kt`

职责：
- 定义全局参数（`GlobalSettings`：间隔/滑动）与配置方案（`ConfigProfile`）数据模型
- 方案中的目标快照 `ProfileTarget` 携带位置与触控类型
- 纯 Kotlin 的 JSON 编解码（`ConfigProfileCodec`），解析失败返回 null 不抛异常，可在 JVM 单测覆盖
